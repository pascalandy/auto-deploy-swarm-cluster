...
echo; echo "Lets open public ports on this node."; echo

DEPLOY_IS_DONE="False"
until [ "$DEPLOY_IS_DONE" = "True" ]
do
    if docker-machine ssh $THIS_NODE -p997 "[ -f "~/temp/node-is-configured.txt" ]" 2> /dev/null; then
        echo "Node $THIS_NODE is deployed" >&2
        docker-machine ssh $THIS_NODE -p997 "ufw allow 80/tcp; ufw allow 443/tcp"
        docker-machine ssh $THIS_NODE -p997 "ufw --force enable; ufw reload; ufw status numbered"
        DEPLOY_IS_DONE="True"
    else
        echo "Node $THIS_NODE is not ready yet. Retry in 2 sec..." >&2
        sleep 2
    fi
done
